...
<%-- Removed fn taglib as it's no longer explicitly needed with simplified logic --%>
<!DOCTYPE html>
<html lang="vi">
    <head>
        <meta charset="UTF-8">
      ...
      <style>
/* ===== PREMIUM BARBER SERVICES STYLESHEET ===== */
                ...
                <div class="services-grid">
                    ...
                    <c:forEach var="service" items="${services}"> <%-- Loop through all services --%>
                        <c:if test="${service.categoryId == 1}"> <%-- Filter by categoryId --%>
                            ...
                            <!-- Example Service Card Structure (áp dụng cho tất cả service cards) -->
    ...
    <button class="add-to-cart"
        ...
        Thêm Vào Đơn
                    ...
                    <c:if test="${not hasServicesInThisCategory}">
                        <p style="color: #666; width: 100%; text-align: center; font-style: italic;">Không có dịch vụ nào trong danh mục này.</p>
                    </c:if>
                </div>
            </section>
...
function loadCartFromSession() {
    ...
    if (storedCart) {
        try {
            ...
            cart.forEach(item => {
                // Fixed the selector - properly escape single quotes
                const button = document.querySelector(`.add-to-cart[data-service-name="${item.name}"]`);
                if (button) {
...
function addToCart(serviceId, serviceName, servicePrice, serviceType) {
    ...
    // Use serviceName for uniqueness check
    ...
    if (isInCart) {
        ...
        // Make sure servicePrice is a number
...
function updateCartSummary() {
    ...
    if (!cartItemCount || !cartTotal || !cartSummary) {
    ...
    if (cart.length > 0) {
        ...
        // Keep it visible but show empty state
        cartSummary.classList.remove('hidden');
        cartSummary.style.display = 'block';
    }
...
function checkout() {
    ...
    if (serviceNamesInput && totalPriceInput) {
