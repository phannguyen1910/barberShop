...
<head>
    <meta charset="UTF-8">
    <title>Dịch Vụ Cắt & Uốn Tóc - Cut&Styles</title>
...
<body>
    ...
    <section class="service-section">
        <div class="container py-5">
            <h2 class="text-center mb-4">Dịch Vụ Nhuộm & Uốn Tóc</h2>
            
            <!-- Phần Cắt Tóc -->
            <div class="row g-4 mb-5">
                <h3 class="mb-3">Nhuộm Tóc</h3>
                <c:set var="hasCutHair" value="false" />
                <c:forEach var="service" items="${hairServices}">
                    <c:if test="${service.categoryID == 3}">
                        <c:set var="hasCutHair" value="true" />
                            ...
                            <div class="service-card h-100">
                                ...
                                <div class="service-images">
                                    <div class="main-image">
                                        <img src="${pageContext.request.contextPath}/images/${service.image}" alt="${service.name}" class="img-fluid rounded">
                                    </div>
                                    <div class="sub-images">
                                        <c:forEach var="img" items="${fn:split(service.image, ',')}" varStatus="status">
                                            <c:if test="${status.index < 2}">
                                                <img src="${pageContext.request.contextPath}/images/${img}" alt="${service.name}" class="img-fluid rounded">
                                            </c:if>
                                        </c:forEach>
                                    </div>
                                </div>
                ...
                <c:if test="${empty hairServices || !hasCutHair}">
                    <div class="col-12 text-center">
                        <p>Chưa có dịch vụ nhuộm tóc nào.</p>
                    </div>
                </c:if>
            </div>
            ...
            <!-- Phần Uốn Tóc -->
            <div class="row g-4">
                <h3 class="mb-3">Uốn Tóc</h3>
                <c:set var="hasCurlHair" value="false" />
                <c:forEach var="service" items="${hairServices}">
                    <c:if test="${service.categoryID == 2}">
                        <c:set var="hasCurlHair" value="true" />
                        <div class="col-md-6 col-lg-4">
                            <div class="service-card h-100">
                                <div class="service-header">
                                    <h3>${service.name}</h3>
                ...
                <c:if test="${empty hairServices || !hasCurlHair}">
                    <div class="col-12 text-center">
                        <p>Chưa có dịch vụ uốn tóc nào.</p>
                    </div>
                </c:if>
            </div>
        </div>
