        ...
        <div class="dashboard-layout">
            ...
            <main class="main-content">
                ...
                <div class="search-section">
                    <div class="search-row">
                        <div class="search-group">
                            ...
                            <input type="date" id="searchDay" name="periodValue" value="${param.periodValue}" class="search-input" onchange="filterData()">
                        </div>
                        <div class="search-group">
                            <label>Tìm kiếm theo tháng:</label>
                            <input type="month" id="searchMonthYear" name="periodValue" value="${param.periodValue}" class="search-input" onchange="filterData()">
                        </div>
                        <div class="search-group">
                            <label>Tìm kiếm theo năm:</label>
                            <input type="number" id="searchYear" name="year" value="${param.year}" class="search-input" placeholder="Nhập năm" min="2000" max="2025" onchange="filterData()">
                        </div>
                        <div class="search-group">
                ...
                <form id="filterForm" method="GET" action="${pageContext.request.contextPath}/RevenueManagementServlet">
                    <input type="hidden" name="branchId" id="formBranchId" value="${param.branchId}">
                </form>
                ...
                <script>
                    // Lưu trữ dữ liệu gốc từ server
                    let originalData = [
                    <c:forEach var="entry" items="${groupedInvoices}" varStatus="status">
                    {
                    period: "${entry.key}",
                            revenue: ${entry.value.revenue != null ? entry.value.revenue : 0},
                            count: ${entry.value.count != null ? entry.value.count : 0}
                    }${status.last ? '' : ','}
                    </c:forEach>
                    ];
                    ...
                    function updateTable(filteredData) {
                        ...
                        if (filteredData.length === 0) {
                            tableBody.innerHTML = '<tr><td colspan="4" style="text-align: center; color: #ccc;">Không có dữ liệu doanh thu.</td></tr>';
                            tableInfo.textContent = 'Hiển thị 0 mục';
                            totalRevenueElement.textContent = '0 VNĐ';
                            return;
                        }

                        let totalRevenue = 0;
                        let totalCount = 0;
                        let periodLabel = 'Thời gian';
                        ...
                        if (searchDay) {
                            periodLabel = 'Ngày';
                        } else if (searchMonthYear) {
                            periodLabel = 'Tháng/Năm';
                        } else if (searchYear) {
                            periodLabel = 'Năm';
                        }

                        periodColumn.textContent = periodLabel;
                 ...
                 <tr>
                     <td>${item.period}</td>
                     <td class="money">${formatNumber(item.revenue)} VNĐ</td>
                     <td>${item.count}</td>
                     <td class="money">${formatNumber(avgRevenue)} VNĐ</td>
                 </tr>
             `;
                        });

                        tableBody.innerHTML = html;
                        tableInfo.textContent = `Hiển thị ${filteredData.length} mục`;
                        totalRevenueElement.textContent = `${formatNumber(totalRevenue)} VNĐ`;
                    }
                    ...
                    function filterData() {
                        ...
                        if (searchDay) {
                            filteredData = filteredData.filter(item => item.period === searchDay);
                        } else if (searchMonthYear) {
                            const [year, month] = searchMonthYear.split('-');
                            filteredData = filteredData.filter(item => {
                                const [itemMonth, itemYear] = item.period.split('/');
                                return itemYear === year && itemMonth === month;
                            });
                        } else if (searchYear) {
                            filteredData = filteredData.filter(item => {
                                if (item.period.includes('/')) {
                                    const [_, itemYear] = item.period.split('/');
                                    return itemYear === searchYear;
                                } else {
                                    return item.period === searchYear;
                                }
                            });
                        }
                    ...
                    function submitForm() {
                        ...
                        document.getElementById('formBranchId').value = searchBranch;
                        form.submit();
                    }
                    ...
                    function resetFilters() {
                        ...
                        updateTable(originalData); // Cập nhật bảng với dữ liệu gốc
                        const form = document.getElementById('filterForm');
                        document.getElementById('formBranchId').value = '';
                        form.submit(); // Gửi yêu cầu reset đến server
                    }

                    window.onload = function () {
                        const today = new Date().toISOString().split('T')[0];
                        document.getElementById('searchDay').max = today;
                        // Gọi filterData ban đầu để hiển thị dữ liệu mặc định
                        filterData();
                    };
