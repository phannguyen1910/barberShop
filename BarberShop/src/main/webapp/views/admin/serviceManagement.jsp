                ...
                <tbody>
                    <c:forEach var="service" items="${services}" varStatus="loop">
                        <tr>
                            ...
                            <td>
                                <c:choose>
                                    <c:when test="${service.categoryID == 1}">Cắt Tóc</c:when>
                                    <c:when test="${service.categoryID == 2}">Uốn Định Hình</c:when>
                                    <c:when test="${service.categoryID == 3}">Thay đổi màu tóc</c:when>
                                    <c:when test="${service.categoryID == 4}">Gội dưỡng sinh thư giãn</c:when>
                                    <c:when test="${service.categoryID == 5}">Lấy ráy tai</c:when>
                            ...
                            <td>
                                <c:if test="${not empty service.image}">
                                    ...
                                    <img src="${pageContext.request.contextPath}/image/${firstImage}" alt="${service.name}" width="50" height="50"/>
                                </c:if>
                            </td>
                            <td>
                                <button class="btn btn-warning me-2" data-bs-toggle="modal" data-bs-target="#editServiceModal"
                                        onclick="showEditModal('${service.id}', '${service.name}', ${service.price}, ${service.duration}, '${service.description}', ${service.categoryID})">
                ...
                <div class="modal-body">
                    <div class="form-group">
                          <label>Loại Dịch vụ</label>
                            <select id="serviceCategoryAdd" name="serviceCategory" class="form-control" placeholder="Nhập tên dịch vụ" required>
                              <option value="dichvu1">Cắt Tóc</option>
                              <option value="dichvu2">Uốn định hình</option>
                              <option value="dichvu3">Thay đổi màu tóc</option>
                              <option value="dichvu4">Gội dưỡng sinh thư giãn</option>
                              <option value="dichvu5">Lấy ráy tai</option>
                              <!-- Thêm lựa chọn mới tại đây -->
                            </select>
                    </div>
            ...
            <form id="editServiceForm" action="${pageContext.request.contextPath}/UpdateService" method="post" enctype="multipart/form-data">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="serviceNameEdit" name="serviceName" class="form-control" required>
                    </div>
                    <div class="form-group">
                        <label>Giá (VNĐ)</label>
                        <input type="number" id="servicePriceEdit" name="servicePrice" class="form-control" min="0" required>
                    </div>
                    <div class="form-group">
                        <label>Thời gian (phút)</label>
                        <input type="number" id="serviceDurationEdit" name="serviceDuration" class="form-control" min="0" required>
                    </div>
                    <div class="form-group">
                        <label>Mô tả</label>
                        <textarea id="serviceDescriptionEdit" name="serviceDescription" class="form-control"></textarea>
                    </div>
                    <div class="form-group">
                        <label>Loại Dịch vụ</label>
                        <select id="serviceCategoryEdit" name="serviceCategory" class="form-control" required>
                            <option value="dichvu1">Dịch vụ 1</option>
                            <option value="dichvu2">Dịch vụ 2</option>
                            <option value="dichvu3">Dịch vụ 3</option>
                            <option value="dichvu4">Dịch vụ 4</option>
                            <option value="dichvu5">Dịch vụ 5</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label>Thư mục ảnh</label>
                        <input type="text" id="imgFolderEdit" name="imgFolder" class="form-control" placeholder="default" value="default">
                    </div>
                    <div class="form-group">
                        <label>Hình ảnh</label>
                        <input type="file" id="serviceImageEdit" name="serviceImage" class="form-control" accept="image/*">
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Hủy</button>
