 ...
 * @author LENOVO
 */
@WebServlet(name = "ForgetPassword", urlPatterns = {"/change-password"})
public class ForgetPassword extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (AccountDAO.updatePassByEmail(pass, email)) {
            session.removeAttribute("otp");
            session.removeAttribute("otpGeneratedTime");
            request.getRequestDispatcher("/views/auth/success.jsp").forward(request, response);
        }else{
