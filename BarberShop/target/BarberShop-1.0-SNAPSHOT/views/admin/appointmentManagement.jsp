    ...
    <head>
        ...
        <style>
            ...
            * {
            ...
            } /* Icons and important text */
            .text-secondary {
                color: #ccc !important;
            } /* Muted text like time */
            .text-success {
    ...
    <body class="bg-dark">
                                ...
                                <div id="addServicesList" class="">
                                    <c:forEach var="service" items="${listService}">
                                        <div class="service-checkbox form-check" style="background: #232323; border-radius: 8px; margin-bottom: 8px; padding: 10px 12px; display: flex; align-items: center;">
                                            ...
                                            <label class="form-check-label" for="addService${service.id}" style="color: #fff; display: flex; align-items: center; gap: 8px; margin-bottom: 0; width: 100%;">
                                                <c:choose>
        ...
        <script>
            ...
            <c:forEach var="appointment" items="${listAppointment}" varStatus="loop">
                            ...
                            params.append('appointmentDate', dateStr.toString());
                            const fullUrl = baseUrl + '?' + params.toString();
                            const response = await fetch(fullUrl);
                            if (!response.ok) return [];
                            return await response.json(); // [{startTime, endTime}, ...]
                            } catch (e) {
                            console.error('Lỗi fetch slot:', e);
                            return [];
                            }
                            }
                            ...
                            let disabledSlots = [];
                            if (staffId && dateStr) {
                            disabledSlots = await fetchDisabledSlots(staffId, dateStr);
                            }
                            ...
                            const startHour = 8, startMinute = 30, endHour = 20, endMinute = 30;
                            let currentSlotTime = DateTime.fromObject({
                            year: selectedDateVN.year, month: selectedDateVN.month, day: selectedDateVN.day,
                            ...
                            const thisSlot = currentSlotTime.hour * 60 + currentSlotTime.minute;
                            ...
                            option.textContent = `${staff.lastName ? staff.lastName : ''} ${staff.firstName ? staff.firstName : ''}`.trim();
